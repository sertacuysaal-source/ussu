...
function App() {
  ...
  const filteredSignals = signals.filter(signal => {
    ...
    return groupSymbols.includes(signal.symbol + ".IS");
  });

  useEffect(() => {
    axios.get(`${API}/groups`)
      .then(res => setGroups(res.data))
      .catch(err => console.error("Gruplar alınamadı:", err));
  }, []);
  ...
  const handleScan = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API}/scan`, { group_name: selectedGroup });
    ...
    <Card
      ...
      style={{
        borderLeftColor:
          ...
          signal.signal === "GÜÇLÜ SAT" ? "#b91c1c" :
          signal.signal === "SAT" ? "#f43f5e" : "#94a3b8"
      }}
    >
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl font-bold text-slate-800">{signal.symbol}</CardTitle>
        ...
        <CardDescription className="flex items-center justify-between mt-2">
          <span className="text-2xl font-semibold text-slate-900">₺{signal.price.toFixed(2)}</span>
          <span className={`text-sm font-medium ${signal.change_percent >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
  ...
  const signalCounts = {
    ...
    "GÜÇLÜ SAT": filteredSignals.filter(s => s.signal === "GÜÇLÜ SAT").length,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="w-full sm:w-auto">
            ...
            >
              ...
              {groups.map((g, idx) => <option key={idx} value={g.name}>{g.name}</option>)}
            </select>
          </div>
        </div>
      </header>
        ...
        {signals.length === 0 ? (
          ...
          <>
            ...
            <Tabs defaultValue="all" className="w-full" onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-6 mb-8 bg-white p-1 shadow-sm" data-testid="signal-tabs">
                <TabsTrigger value="all" className="data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">Tümü ({signalCounts.all})</TabsTrigger>
                <TabsTrigger value="GÜÇLÜ AL" className="data-[state=active]:bg-amber-500 data-[state=active]:text-white transition-all">Güçlü Al ({signalCounts["GÜÇLÜ AL"]})</TabsTrigger>
                <TabsTrigger value="AL" className="data-[state=active]:bg-emerald-500 data-[state=active]:text-white transition-all">Al ({signalCounts.AL})</TabsTrigger>
                <TabsTrigger value="SAT" className="data-[state=active]:bg-rose-500 data-[state=active]:text-white transition-all">Sat ({signalCounts.SAT})</TabsTrigger>
                <TabsTrigger value="GÜÇLÜ SAT" className="data-[state=active]:bg-red-700 data-[state=active]:text-white transition-all">Güçlü Sat ({signalCounts["GÜÇLÜ SAT"]})</TabsTrigger>
                <TabsTrigger value="TUT" className="data-[state=active]:bg-slate-400 data-[state=active]:text-white transition-all">Tut ({signalCounts.TUT})</TabsTrigger>
              </TabsList>

              {["all", "GÜÇLÜ AL", "AL", "SAT", "GÜÇLÜ SAT", "TUT"].map((tab) => (
                <TabsContent key={tab} value={tab} className="mt-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" data-testid={`${tab}-signals-grid`}>
                    {filteredSignals
                      ...
                      .map((signal, idx) => <SignalCard key={idx} signal={signal} />)}
                  </div>
                  {filteredSignals
                    .filter((signal) => tab === "all" || signal.signal === tab)
                    .filter((signal) => signal.symbol.toLowerCase().includes(searchText.toLowerCase()))
